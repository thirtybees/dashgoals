...
function dashgoals_changeYear(xward) {
  var newYear = dashgoals_year;
  if (xward === 'forward') {
    newYear = dashgoals_year + 1;
  ...
  $.ajax({
    ...
    success: function (result) {
      $('#dashgoals_title').text($('#dashgoals_title').text().replace(dashgoals_year, newYear));
      var hideConf = $('#dashgoals_config').hasClass('hide');
